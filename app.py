...
# ==========================================
# FIX #1: width="stretch" não é válido → use use_column_width=True
...
dicionario_cas = {
    ...
    "7440-22-4": {"agente": "Prata (Compostos)",         "nr15_lt": "0,01 mg/m³",               "nr09_acao": "0,005 mg/m³",           "nr07_ibe": "Avaliação Clínica",                             "dec_3048": "Não Enquadrado",         "esocial_24": "09.01.001"},
...
def buscar_nomes_faltantes_ia(texto_fispq, cas_faltantes):
    """FIX #3: Parsing robusto do JSON com limpeza de markdown e try/except."""
    ...
    Retorne APENAS um objeto JSON puro (sem markdown, sem explicações) onde a chave é o número CAS e o valor é o nome do produto químico.
    ...
    try:
        ...
        resposta = requests.post(url_google, headers={'Content-Type': 'application/json'}, json=payload, timeout=30)
        ...
        if resposta.status_code == 200:
            ...
            st.warning(f"⚠️ IA retornou erro {resposta.status_code} para CAS desconhecidos.")
    except json.JSONDecodeError as e:
        st.warning(f"⚠️ IA retornou um JSON malformado para os CAS desconhecidos. Os agentes serão marcados para revisão manual.")
    except Exception as e:
        st.warning(f"⚠️ Falha na consulta IA para CAS: {e}")
    return {}
...
def gerar_html_anexo(resultados_pgr, resultados_medicos):
    ...
    for ghe in ghes:
        ...
        if not med_ghe.empty:
            html_content += "<h4>⚕️ Diretrizes Médicas e Previdenciárias</h4><table class='funcao-mini-table'><thead><tr><th>Cód / CAS</th><th>Agente</th><th>Lim. Tol. (NR-15)</th><th>Nível Ação (NR-09)</th><th>Exame/IBE (NR-07)</th><th>Dec 3048</th><th>eSocial</th></tr></thead><tbody>"
...
# ── DASHBOARD ────────────────────────────────────────────────────────────────
    ...
    col1.metric("📁 Total de Laudos Salvos", len(df_dash))
    pgr_count  = len(df_dash[df_dash['nome_projeto'].str.contains('PGR',  na=False)])
    pcmso_count= len(df_dash[df_dash['nome_projeto'].str.contains('PCMSO',na=False)])
    col2.metric("⚙️ Relatórios PGR",   pgr_count)
    col3.metric("🩺 Relatórios PCMSO", pcmso_count)
...
# ── MÓDULO 1: ENGENHARIA ─────────────────────────────────────────────────────
    ...
    st.info("A IA foi ativada neste módulo. Caso uma substância química (CAS) não esteja no banco de dados local, o motor lerá o nome químico diretamente da FDS via Gemini.")
    ...
    if arquivos_fispq:
        with st.spinner("Lendo o conteúdo dos PDFs..."):
            for arquivo in arquivos_fispq:
                # FIX #2: usar io.BytesIO para evitar problema de buffer consumido
    ...
    # FIX #1b: width="stretch" → use_container_width=True
        ...
        with st.spinner("Consolidando avaliações químicas com IA..."):
            ...
            if not df_editado.empty:
                for _, row in df_editado.iterrows():
                    ...
                    nomes_ia = buscar_nomes_faltantes_ia(texto_completo, cas_desconhecidos)

                    for cas in cas_encontrados:
                        if cas in dicionario_cas:
                            dados_med = dicionario_cas[cas]
                        else:
                            nome_ia = nomes_ia.get(cas, "Produto Químico (Avaliar FDS)")
                            dados_med = {
                                "agente":     nome_ia,
...
# ── MÓDULO 2: MEDICINA ────────────────────────────────────────────────────────
    ...
    if arquivo_pgr:
        ...
        # FIX #1c: use_container_width no lugar de width/use_container_width errado
        if st.button("🚀 Extrair Riscos e Gerar PCMSO", type="primary", use_container_width=True):
            with st.spinner("Motor IA analisando o documento e cruzando protocolos médicos... Aguarde até 1 minuto."):
                ...
                # FIX #2b: getvalue() não consome o buffer
                ...
                try:
                    # Detecção dinâmica do melhor modelo disponível
                    ...
                    modelo_escolhido = "models/gemini-1.5-flash"  # fallback seguro
                    ...
                    if resposta.status_code == 200:
                        resultado_texto = resposta.json()['candidates'][0]['content']['parts'][0]['text']
                        # FIX #3b: limpeza robusta + json.loads com tratamento de erro
                        resultado_limpo = limpar_json_ia(resultado_texto)

                        try:
                            json_pgr = json.loads(resultado_limpo)
                        except json.JSONDecodeError:
                            # Tentativa de recuperação: isolar apenas o bloco JSON
